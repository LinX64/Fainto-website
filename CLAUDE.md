...
**CDN cache trap.** `fainto.app` is fronted by Cloudflare, which caches `styles.css`/`app.js` at the edge for 4h (`cf-cache-status: HIT` even with `must-revalidate`). HTML revalidates fine, but a changed CSS/JS keeps serving the **old** version for hours after a green deploy — the origin (`66.29.141.97`) is correct, the edge is stale. When you change `styles.css` or `app.js`, **bump the `?v=YYYYMMDD` query on the `<link rel="stylesheet">` / `<script src>` across every page** (cheap, deterministic, new cache key) or purge the Cloudflare cache. After deploy, verify a token from the new CSS is live through the CDN (e.g. `curl -s https://fainto.app/styles.css | grep <new-rule>`), not just at the origin.
